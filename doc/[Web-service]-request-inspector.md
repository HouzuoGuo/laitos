...
the endpoint dumps the incoming HTTP request for your inspection - this includes
the request headers, request body, etc.

This often comes in handy as a diagnosis tool for deploying laitos web server
behind intermediary components, such as a load balancer or an API gateway, which
often manipulates client's HTTP requests by changing the headers and/or request
body.

## Configuration

Under the JSON key `HTTPHandlers`, add a string property called
`RequestInspectorEndpoint`, value being the URL location of the service.
