...
func LauncherUtilityExtract(destDir, archivePath string) {
	...
	*/
	...
	fmt.Println("Result is (nil means success): ", ExtractArchive(archivePath, tmpFile.Name(), destDir, password))
}
...
func LauncherUtilityArchive(srcDir, archivePath string) {
	...
	password = []byte(strings.TrimSpace(string(password)))
	fmt.Println("Result is (nil means success): ", MakeArchive(srcDir, archivePath, password))
}
