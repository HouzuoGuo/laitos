...
func (hand *HandleMicrosoftBot) RetrieveJWT() (jwt MicrosoftBotJwt, err error) {
	...
	httpResp, err = httpclient.DoHTTP(httpclient.Request{
...
func (hand *HandleMicrosoftBot) MakeHandler(logger global.Logger, cmdProc *common.CommandProcessor) (http.HandlerFunc, error) {
	...
	fun := func(w http.ResponseWriter, r *http.Request) {
		...
		go func() {
			...
			if !messageTime.After(global.StartupTime.UTC()) {
			...
			result := cmdProc.Process(feature.Command{TimeoutSec: MicrosoftBotCommandTimeoutSec, Content: incoming.Text})
			...
			// Send away the reply
			_, err = httpclient.DoHTTP(httpclient.Request{
				Method:      http.MethodPost,
