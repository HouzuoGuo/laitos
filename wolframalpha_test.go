...
func TestWolframAlphaInvokeAPI(t *testing.T) {
	t.Skip()
	run := CommandRunner{PIN: "abc", TimeoutSec: 12, TruncateLen: 500, WolframAlpha: WolframAlphaClient{AppID: "FILLME"}}
	output := run.RunCommand(run.FindCommand("abc#wweather in nuremberg, bavaria, germany"), true)
	t.Log(output)
	if len(output) < 50 {
...
func TestWolframAlphaExtractResponse(t *testing.T) {
