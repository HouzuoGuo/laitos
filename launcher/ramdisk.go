...
func MakeRamdisk(sizeMB int) (string, error) {
	...
	out, err := env.InvokeShell(RamdiskCommandTimeoutSec, "/bin/sh", fmt.Sprintf("mount -t tmpfs -o size=%dm tmpfs '%s'", sizeMB, mountPoint))
	if err != nil {
...
func DestroyRamdisk(mountPoint string) {
	out, err := env.InvokeShell(RamdiskCommandTimeoutSec, "/bin/sh", fmt.Sprintf("umount -lfr '%s'", mountPoint))
	if err != nil {
		// Retry once
