...
func TestPINAndShortcuts_Transform(t *testing.T) {
	...
	if out, err := pin.Transform(toolbox.Command{Content: "abc"}); err != ErrPINAndShortcutNotFound || out.Content != "abc" {
		t.Fatal(out, err)
	}
	...
	if out, err := pin.Transform(toolbox.Command{Content: "nothing_to_see"}); err != ErrPINAndShortcutNotFound || out.Content != "nothing_to_see" {
		t.Fatal(out, err)
	}
	if out, err := pin.Transform(toolbox.Command{Content: "\n\n mypineapple \n\n"}); err != nil || out.Content != "eapple" {
		t.Fatal(out, err)
	...
	if out, err := pin.Transform(toolbox.Command{Content: "ghi"}); err != ErrPINAndShortcutNotFound || out.Content != "ghi" {
		t.Fatal(out, err)
	}
}
